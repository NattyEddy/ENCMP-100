...
## Occult Circle
#  @param im - image data
#  @param width - desired diameter of circle
#  @return im - altered image data
#  @return mask - data of pixels altered with the black circle (boolean)
#  take an image and place a circle of desired diameter in the center
def occultCircle(im,width):
    ...
    for x in range(im.shape[0]//2 - width//2, im.shape[0]//2 + width//2):    
        for y in range(im.shape[1]//2 - width//2, im.shape[1]//2 + width//2):
            if np.sqrt((x - im.shape[0]//2)**2 + (y - im.shape[1]//2)**2) <= width//2:
                im[x, y] = 0
                mask[x, y] = True
    return (im, mask)
